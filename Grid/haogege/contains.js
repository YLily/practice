...
function(require) {
    ...
    var findById = function(parent, id) {
        var append = function(host, tail) {
            for (var i = 0,
            len = tail.length; i !== len; host.push(tail[i++])) {}
            return host
        },
    ...
    SelectPanel.prototype = {
        render: function() {
            ...
            events.addEvent(findById(frame, "WPA3-SELECT-PANEL-CLOSE"), "click",
            function() {
                panel.remove();
                opts.onClose && opts.onClose()
            });
            events.addEvent(findById(frame, "WPA3-SELECT-PANEL-AIO-CHAT"), "click",
            function() {
                panel.remove();
                opts.onAIOChat && opts.onAIOChat()
            });
            events.addEvent(findById(frame, "WPA3-SELECT-PANEL-ANONY-CHAT"), "click",
            function() {
                panel.remove();
...
BizQQWPA.define("lang.extend", "lang.each,lang.typeEnhance",
function(require) {
