function check_dstype(a) {
	...
	if (c[b] && c[b] != a && (!sessionStorage.dstyp_l || sessionStorage.dstyp_l != a)) {
...
function dssend_ori(a, b) {
	...
	if ("withCredentials" in c) {
		...
		document.cookie = "daref=" + document.referrer + ";domain=.bilibili.com";
		rec_jsonp(d, true)
	}
}
function dssend(a, b) {
...
function dshref(a) {
	if (a.match(/\/video\/av\d+/) != null) return a.match(/av\d+/).toString();
	else if (a.match(/\/topic\/\d+/) != null || a.match(/\/topic\/v2\/\d+/) != null) return "topic" + a.match(/(\d+)\.html/)[1];
	else if (a.match(/acg\.tv\/([a-zA-Z]+)/) != null) return "acg_tv_" + a.match(/acg\.tv\/([a-zA-Z]+)/)[1];
	else if (a.match(/space\.bilibili\.com\/(\d+)/) != null) return "space" + a.match(/space\.bilibili\.com\/(\d+)/)[1];
	else if (a.match(/live\.bilibili\.com\/(\d+)/) != null) return "lv" + a.match(/live\.bilibili\.com\/(\d+)/)[1];
...
(function(a, b, c, d) {
	...
	function r(a, b) {
		...
		return location.protocol + "//" + d + "?" + q(b) + "&_=" + (new Date).getTime()
	}
	function s(a, b) {
...
(function(a, b) {
	...
	}).on("mouseover", ".i-link", function() {
		var a = b(this).parent().index();
		var c = b(this).parent().attr("id");
		if (a !== 0 && c !== "i_menu_login_reg" && b(this).data("show") != "re") {
			if (a == 6) a = 5;
			g(a + 1);
			b(this).data("show", "re")
		}
...
(function(a, b, c) {
	if (!a || location.hostname != "message.bilibili.com") return;
	var d = {
		optype: "",
		messagesid: "",
		clickitem: ""
	...
	function g(a) {
		...
		rec_rp("home_notice", c)
	}
	b(c).on("click", ".J_black", function() {
		g(1)
	}).on("click", ".J_cancel_black", function() {
		g(2)
	}).on("click", ".chat_close", function() {
		g(3)
	});
	b(c).ajaxComplete(function(a, c, d) {
		var g, h;
		if (d.url.indexOf("/api/notify/query.replyme.list.do") >= 0 || d.url.indexOf("/api/notify/query.atme.list.do") >= 0 || d.url.indexOf("/api/notify/query.sysnotify.list.do") >= 0) {
			g = b.parseJSON(c.responseText);
			if (d.url.indexOf("cursor") > 0) {
				h = 2
			} else {
				h = 1
			}
...
})(window.dataRec, window.jQuery, document);
(function(a, b) {
	if (!a || location.hostname != "bangumi.bilibili.com" || !a.sampling("5", 10)) return;
	var c = b.onload;
	b.onload = function() {
		...
		if (d) {
			if (!d["loadEventEnd"]) d["loadEventEnd"] = (new Date).getTime();
